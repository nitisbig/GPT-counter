...
/**
 * GPT Deadline – Single-file React + TypeScript component
 * Tech: React, TypeScript, Tailwind, Framer Motion, Luxon
 * Works as a drop-in <GPTDeadlineApp /> component.
 ...
 * - Reminders (local Notification API + in-app toasts) with toggles & snooze
 * - Settings: edit cycle, 12/24h, theme, .ics export, QA "set Now" override
 * - Inline service worker registration for basic offline support (PWA hint)
 * - Privacy: stores only in localStorage; no network calls
 *
 * NOTE: This file assumes TailwindCSS is available globally.
 */
...
export type SubscriptionCycle = {
  ...
  startISO: string; // e.g., "2025-08-20T07:18:00+05:45"
  endISO: string;   // e.g., "2025-09-20T07:18:00+05:45"
  reminders: {
    halfway: boolean;
    threeDays: boolean;
    lastDay: boolean;
...
const DEFAULT_ZONE = "Asia/Kathmandu" as const; // Persist & use for all calcs
...
function inZone(iso: string, zone = DEFAULT_ZONE) {
  return DateTime.fromISO(iso, { zone });
}

function fmtDate(dt: DateTime, hourFormat: HourFormat) {
...
function fmtTime(dt: DateTime, hourFormat: HourFormat) {
  const opts: Intl.DateTimeFormatOptions = {
    hour: "numeric",
    minute: "2-digit",
    hour12: hourFormat === "12h",
...
function rollCycleToNow(cycle: SubscriptionCycle, now: DateTime): { start: DateTime; end: DateTime; rolled: boolean } {
...
function useNowTick(qaMode: boolean, qaNowISO?: string) {
  ...
  useEffect(() => {
    let frame = 0;
    const tick = () => setNow((prev) => qaMode ? prev.plus({ seconds: 1 }) : DateTime.now().setZone(DEFAULT_ZONE));
    const id = setInterval(tick, 1000);
    return () => { cancelAnimationFrame(frame); clearInterval(id); };
  }, [qaMode]);
  useEffect(() => {
    if (qaMode && qaNowISO) setNow(inZone(qaNowISO));
  }, [qaMode, qaNowISO]);
  return now;
}

function useReminders(
  enabled: SubscriptionCycle["reminders"],
  start: DateTime,
  end: DateTime,
) {
  ...
  const snooze = (minutes = 60) => {
    ...
    const id = window.setTimeout(() => {
      if ("Notification" in window && Notification.permission === "granted") {
        new Notification("Reminder", { body: "This is your snoozed reminder." });
      }
...
function CalendarStrip({ start, now, end }: { start: DateTime; now: DateTime; end: DateTime }) {
  const days: { date: DateTime; fill: number }[] = [];
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl p-4 bg-white/70 dark:bg-white/5 shadow-sm ring-1 ring-black/5 dark:ring-white/10">
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button className="px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-sm" onClick={onResetDefaults}>Reset to defaults</button>
            </div>
          </div>
